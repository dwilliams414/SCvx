...
final_state_error = max(abs(xkp1mex-xkp1_ref), [], 'all')

%% Errors with respect to nonlinear
x0_pert = x0 + randn(6, 1)*1e-4;
uk_pert = uk + randn(3, 1)*1e-4;
...
final_state_diff_error = final_state_diff_actual-final_state_diff_pred

final_state_error = xnl(end, :)'-(Akmex*x0_pert+Bkmex*uk_pert+ckmex)